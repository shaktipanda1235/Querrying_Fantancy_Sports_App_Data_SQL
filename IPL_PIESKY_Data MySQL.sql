...
CREATE DATABASE IF NOT EXISTS ipl_miniproject_1;
USE ipl_miniproject_1;
...
select bidder_id,((if(bid_status='won',cnt,0))/sum(cnt))*100 win_prcnt
from
(select bidder_id,bid_status,count(bid_status) as cnt
from ipl_bidding_details
where bid_status in ('won',"lost")
group by BIDDER_ID,BID_STATUS)t
group by bidder_id
order by win_prcnt desc;
...
select ims.stadium_id, stadium_name,city, count(schedule_id) as no_of_matches
from ipl_match_schedule ims
join ipl_stadium is1
on ims.stadium_id=is1.STADIUM_ID
...
select STADIUM_NAME,((winner/total)*100) prcentage_win_after_winning_toss
from

(select stadium_name ,count(ims.MATCH_ID) total
...
(select stadium_name ,count(ims.MATCH_ID) winner
from ipl_match im
join ipl_match_schedule ims
on ims.MATCH_ID=im.MATCH_ID
join ipl_stadium is1
...
select match_id,new_winner_id,team_name winner_team_name,WIN_DETAILS
from
(select *,
(case 
	when new_winner_id in
    (select team_id from ipl_team
    where length(remarks)=3)
	then substr(win_details,6,3)
	when new_winner_id in
	(select team_id from ipl_team
    where length(remarks)=2)
	then substr(win_details,6,2)
	else substr(win_details,6,4)
   
    end) as tag1
from 
(select *,
if(match_winner=1,team_id1,team_id2)  new_winner_id
 from ipl_match)t)t2
...
select team_id1,team_name,total_matches,win_count,loss_count
from
(select team_id1,total_matches,win_count,(total_matches-win_count) loss_count
from
(select * from
(select team_id1, (count(team_id1)+count(team_id2)) total_matches
from ipl_match
group by team_id1)t
 
 join
(select new_winner_id as team_id1 ,count(new_winner_id) win_count
from
(select *,
if(match_winner=1,team_id1,team_id2)  new_winner_id
 from ipl_match)t
 group by new_winner_id)t2
using (team_id1))t4)t5
join ipl_team
on team_id=team_id1;
...
select itp.PLAYER_ID,player_name,itp.PLAYER_ROLE 
...
select itp.team_id,team_name,count(itp.player_id) cnt_of_allrounders
from ipl_team_players itp
join ipl_team it
on itp.TEAM_ID=it.TEAM_ID
where player_role='All-rounder'
group by itp.team_id
having cnt_of_allrounders>4
order by cnt_of_allrounders desc;
